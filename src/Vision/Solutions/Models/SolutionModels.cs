...
public sealed class SolutionManager
{
  ...
  foreach (var st in sol.Stations.Where(st => st != null))
  {
   try
   {
    ...
    if (st.DetectionTool?.ToolBlock != null)
    {
     ...
     try { SaveToolVarsSidecar(sol, st, st.DetectionTool); } catch (Exception ex) { LogHelper.Warn($"工位[{st.Name}]检测工具变量保存失败: {ex.Message}"); }
    }
   }
   catch (Exception ex)
   {
  ...
  foreach (var st in sol.Stations.Where(st => st != null))
  {
   try
   {
    ...
    if (st.DetectionTool != null)
    {
     ...
     try { LoadToolVarsSidecar(sol, st, st.DetectionTool); } catch (Exception ex) { LogHelper.Warn($"工位[{st.Name}]检测工具变量加载失败: {ex.Message}"); }
    }
   }
   catch (Exception ex)
   {
 ...
 /// 获取工具的 .vpp 完整路径。
 /// </summary>
 public static string GetToolVppPath(Solution sol, StationConfig st, StationConfig.ToolBase tool)
 {
 ...
 /// </summary>
 public static string GetToolVarsPath(Solution sol, StationConfig st, StationConfig.ToolBase tool)
 {
  var folder = GetStationFolder(sol, st);
  if (string.IsNullOrEmpty(folder)) return null;
  var file = tool?.FileName;
 ...
 /// </summary>
 private static void SaveToolVarsSidecar(Solution sol, StationConfig st, StationConfig.ToolBase tool)
 {
  if (tool == null) return;
 ...
 /// </summary>
 private static void LoadToolVarsSidecar(Solution sol, StationConfig st, StationConfig.ToolBase tool)
 {
  if (tool == null) return;
  var varsPath = GetToolVarsPath(sol, st, tool);
