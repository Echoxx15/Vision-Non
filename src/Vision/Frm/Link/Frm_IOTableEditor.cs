...
var btnSave = new ToolStripButton("保存") { DisplayStyle = ToolStripItemDisplayStyle.Text, ToolTipText = "保存当前配置且不关闭窗口" };
...
var btnRefreshOutputs = new ToolStripButton("刷新输出") { DisplayStyle = ToolStripItemDisplayStyle.Text, ToolTipText = "从检测工具刷新可用的输出列表" };
...
_dgvVars.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = nameof(IOVariableConfig.Description), HeaderText = "说明", Width =160 });
...
var colAcc = new DataGridViewComboBoxColumn { DataPropertyName = nameof(IOVariableConfig.AccessType), HeaderText = "方向", Width =70, DisplayStyle = DataGridViewComboBoxDisplayStyle.DropDownButton };
...
// 输出变量名为下拉列表，从 ToolBlock.Outputs 动态获取
...
Title = "导入变量"
 ...
 if (lines.Length < 2)
 {
MessageBox.Show("CSV文件格式不正确", "警告", MessageBoxButtons.OK, MessageBoxIcon.Warning);
 return;
 }
...
// 仅 Output 或 InputOutput 方向才需要输出变量
...
// 从 ToolBlock 获取输出列表
 ...
 {
var list = new List<string> { string.Empty }; // 可不选
...
MessageBox.Show($"已刷新输出列表，共 {outputNames.Count - 1} 个可用项", "刷新完成", MessageBoxButtons.OK, MessageBoxIcon.Information);
 }
