...
public abstract class CommAdapterBase : IComm, IRenameableComm, IConfigurableComm
{
 ...
 Console.WriteLine($"[{Name}] Table状态: 输入{inputCount}项, 输出{outputCount}项");
 ...
 Console.WriteLine($"[{Name}] ? TableConfig已创建: 输入{config.TableConfig.Inputs.Count}项, 输出{config.TableConfig.Outputs.Count}项");
 }
 else
 {
 ...
 Console.WriteLine($"[{Name}] ? 通讯表加载完成: 输入{inputCount}项, 输出{outputCount}项");
 }
 else
 {
...
protected virtual void OnMessageReceived(object message)
{
 ...
 catch (Exception ex) { Console.WriteLine($"[{Name}] 消息接收事件处理异常: {ex.Message}"); }
}

protected virtual void OnConnectionStatusChanged(bool connected)
{
 try { ConnectionStatusChanged?.Invoke(this, connected); }
 catch (Exception ex) { Console.WriteLine($"[{Name}] 连接状态变化事件处理异常: {ex.Message}"); }
}
