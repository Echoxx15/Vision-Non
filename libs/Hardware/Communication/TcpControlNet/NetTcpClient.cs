...
/// - async/await 架构
/// - 自动重连（可关闭）
/// - 统一通过 MessageReceived 发布消息
/// - 统一通过 ConnectionStatusChanged 发布连接状态
/// - 简单可控的换行分帧（可选）
/// </summary>
public sealed class NetTcpClient : IDisposable
{
  ...
  /// <summary>原始字节接收事件（保留用于特殊场景）</summary>
        ...
        try
        {
          ...
          if (FrameByNewLine)
          {
            ...
            while ((idx = acc.IndexOf('\n')) >= 0)
            {
              ...
              try
              {
                LogHelper.Info($"[{Name}] 接收到消息: {line}");
              }
              catch
              {
                // 日志失败不影响消息处理
          ...
          else
          {
            ...
            try
            {
              LogHelper.Info($"[{Name}] 接收到消息块: {text.Length} 字节");
            }
            catch
            {
              // 日志失败不影响消息处理
