...
public partial class Frm_Tool : Form
{
  ...
  private BindingList<ProcessStation.DetectVarDef> _vars; // 绑定到 Detection.Vars
  private string _originalNameEditing;   // 进入编辑前的名称，用于校验/回滚
  private string _originalValueEditing;  // 进入编辑前的值，用于校验/回滚

  private ProcessStation.ToolBase Detection { get; set; }
  ...
  public Frm_Tool(bool panel1Collapsed = true)
  {
    InitializeComponent();
    InitDgv();

    // 初始时是否折叠左侧面板
    splitContainer1.Panel1Collapsed = panel1Collapsed;
  ...
  public void LoadDetection(ProcessStation.ToolBase det)
  {
    Detection = det;
    _vars = new BindingList<ProcessStation.DetectVarDef>(Detection.Vars ?? (Detection.Vars = []));
  ...
  private void InitDgv()
  {
    ...
    {
      DataPropertyName = nameof(ProcessStation.DetectVarDef.TypeName), Name = "类型", HeaderText = "类型", ReadOnly = true,
      Width = 60
    });
    // 名称列（可编辑）
    dgv_Data.Columns.Add(new DataGridViewTextBoxColumn
    {
      DataPropertyName = nameof(ProcessStation.DetectVarDef.Name), Name = "名称", HeaderText = "名称", ReadOnly = false,
      Width = 120
    });
    // 值列（可编辑，数组不允许直接编辑）
    dgv_Data.Columns.Add(new DataGridViewTextBoxColumn
    {
      DataPropertyName = nameof(ProcessStation.DetectVarDef.Value), Name = "值", HeaderText = "值", ReadOnly = false,
      Width = 100
    });
    // 注释列（自适应填充）
    var noteCol = new DataGridViewTextBoxColumn
    {
      DataPropertyName = nameof(ProcessStation.DetectVarDef.Comment), Name = "注释", HeaderText = "注释", ReadOnly = false,
    ...
    dgv_Data.CellBeginEdit += (_, e) =>
    {
      ...
      {
        case nameof(ProcessStation.DetectVarDef.Name):
          _originalNameEditing = dgv_Data.Rows[e.RowIndex].Cells[e.ColumnIndex].Value?.ToString();
          break;
        case nameof(ProcessStation.DetectVarDef.Value):
  ...
  private void AddVar(Type t)
  {
    ...
    _vars.Add(new ProcessStation.DetectVarDef
  ...
  private void dgv_Data_CellEndEdit(object sender, DataGridViewCellEventArgs e)
  {
    if (e.RowIndex < 0 || e.ColumnIndex < 0) return;
    var col = dgv_Data.Columns[e.ColumnIndex];
    if (dgv_Data.Rows[e.RowIndex].DataBoundItem is not ProcessStation.DetectVarDef r) return;
    if (col.DataPropertyName == nameof(ProcessStation.DetectVarDef.Name))
    {
    ...
    else if (col.DataPropertyName == nameof(ProcessStation.DetectVarDef.Value))
    {
