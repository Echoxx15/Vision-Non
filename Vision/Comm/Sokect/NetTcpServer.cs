...
public sealed class NetTcpServer : IDisposable
{
    ...
    private async Task AcceptLoopAsync(CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                {
                    try { var h = ClientConnected; if (h != null) h(id); } catch { }
            ...
            catch { try { await Task.Delay(200, token).ConfigureAwait(false); } catch { break; } }
        }
    }
    ...
    private async Task ReceiveLoopAsync(Client c, CancellationToken token)
    {
        ...
        try
        {
            while (!token.IsCancellationRequested)
            {
                ...
                var bytesHandler = BytesReceived; if (bytesHandler != null) bytesHandler(c.Id, buf, n);
                ...
                if (FrameByNewLine || textHandler != null)
                {
                    ...
                    if (FrameByNewLine)
                    {
                        ...
                        while ((idx = acc.IndexOf('\n')) >= 0)
                        {
                            ...
                            LogHelper.Info($"收到客户端[{c.Id}] ，消息[{line}]");
                    ...
                    else
                    {
                        if (textHandler != null)
                        {
                            LogHelper.Info($"收到客户端[{c.Id}] ，消息[{text}]");
    ...
    public void Broadcast(string text)
    {
        ...
        {
            try { kv.Value.Socket.Send(data); } catch { }
        }
    }
    ...
    {
        try { c.Socket.Shutdown(SocketShutdown.Both); } catch { }
        try { c.Socket.Close(); } catch { }
        Client tmp; _clients.TryRemove(c.Id, out tmp);
        try { var h = ClientDisconnected; if (h != null) h(c.Id); } catch { }
    }

    public void Stop()
    {
        var t = _cts; _cts = null;
        try { t?.Cancel(); } catch { }
